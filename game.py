...
from utilities import prep_perfdict, valid_moves
...
def game_step(move,
    ...
    if move == 'A':
        ...
        if PRINT:
            print "Player A's turn, cards are:", A
        # drop = int(raw_input())
        ...
        try:
            # print "trying this key :::",
            # (str(sorted(choices)),str(KNOWNS),HMAID,str(deck))
            strat_dict = strategy[(str(sorted(choices)), str(KNOWNS), HMAID,
                                   str(deck))]
        except:
            strat_dict = {}
        perf_dict = prep_perfdict(drop_ops, strat_dict)
        if PRINT:
            print "Perf-dict from known experiments:", perf_dict
        ...
        if drop == known_B or drop / 10 == known_B:
            known_B = 0
        if PRINT:
            print "Card chosen to drop:", drop
        A, B, CARDS, hmaid_A, known_A, known_B, win_flag, xtr_drop = \
            get_action('A', A, B, CARDS, drop, hmaid_B, known_A, known_B)

    else:
        B.append(CARDS.pop(0))
        choices = B[:]
        if PRINT:
            print "Player B's turn, your cards: ", B
            print "Enter the card that you'd like to drop: ",
        drop = int(raw_input())
        ...
        try:
            # print "trying this key :::", \
			# (str(sorted(choices)),str(KNOWNS),HMAID,str(deck))
            strat_dict = strategy[(str(sorted(choices)),
								  str(KNOWNS), HMAID, str(deck))]
        except:
            strat_dict = {}
        perf_dict = prep_perfdict(drop_ops, strat_dict)
        ...
        if PRINT:
            print "Card chosen to drop:", drop
        A, B, CARDS, hmaid_B, known_A, known_B, win_flag, xtr_drop = \
            get_action('B', A, B, CARDS, drop, hmaid_A, known_A, known_B)
    ...
    return (A, B, CARDS, SEEN, hmaid_A, hmaid_B, known_A, known_B, win_flag,
            (move, str(choices), str(KNOWNS), HMAID, str(deck), drop))
...
def game_setup():
    ...
    return (CARDS, SEEN, A, B, hmaid_A, hmaid_B, known_A, known_B, win_flag,
            game_states, cnt)
...
def test_step(move,
    ...
    if move == 'A':
        ...
        if drop == known_B or drop / 10 == known_B:
            known_B = 0
        A, B, CARDS, hmaid_A, known_A, known_B, win_flag, xtr_drop = get_action(
            'A', A, B, CARDS, drop, hmaid_B, known_A, known_B)
        ...
        if drop == known_A or drop / 10 == known_A:
            known_A = 0
        A, B, CARDS, hmaid_B, known_A, known_B, win_flag, xtr_drop = get_action(
            'B', A, B, CARDS, drop, hmaid_A, known_A, known_B)
